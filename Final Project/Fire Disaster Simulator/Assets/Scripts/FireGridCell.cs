...
public class FireGridCell : MonoBehaviour
{
    ...
    private FireBoxSpread m_fireBox = null; // used to detect collision with GameObjects that have colliders
    ...
    public void SetupCell(bool alight, GameObject fire, CellAttributes cellAttribute, string groundName, Vector2[] firesPositionsInCell)
    {
        m_isAlight = alight;
        m_firePrefab = fire;
        m_fires = new GameObject[firesPositionsInCell.Length];
        m_firesPositions = firesPositionsInCell;
        m_HP = cellAttribute.HP;
        m_fuel = cellAttribute.fuel;
        m_extinguishThreshold = cellAttribute.fuel * cellAttribute.threshold;
        m_moisture = cellAttribute.moisture;
        m_fireBox = new FireBoxSpread();
        m_fireBox.Init(transform.position, groundName);
        float boxExtents = cellAttribute.cellSize / 2.0f;
        m_fireBox.radius = new Vector3(boxExtents, boxExtents, boxExtents);
        m_combustionConstant = cellAttribute.combustionValue;
        SetInitialFireValues(cellAttribute.airTemperature, cellAttribute.propagationSpeed);
        m_persist = cellAttribute.Persist;
    }
    ...
    {
        m_ignitionTemperature = m_HP  + m_moisture;
